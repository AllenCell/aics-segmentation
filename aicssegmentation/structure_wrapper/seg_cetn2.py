...
def Workflow_cetn2(
    ...
    # rescale if needed
    if rescale_ratio > 0:
        struct_img_for_seg = resize(
            struct_img_for_seg, [1, rescale_ratio, rescale_ratio], method="cubic"
        )
        struct_img_for_seg = (struct_img_for_seg - struct_img_for_seg.min() + 1e-8) / (
            struct_img_for_seg.max() - struct_img_for_seg.min() + 1e-8
        )
        struct_img_for_peak = resize(
            struct_img_for_peak, [1, rescale_ratio, rescale_ratio], method="cubic"
        )
        struct_img_for_peak = (
    ...
    if output_type == "default":
        ...
        img_list, name_list = CETN2_output(
