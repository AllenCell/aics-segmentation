...
def Workflow_polr2a_blob_v2(
    ...
    bw = remove_small_objects(bw > 0, min_size=blob_min_area, connectivity=1, in_place=False)
    ...
    bw_masked = np.logical_and(im_watershed, bw_otsu_mask_removesmallobjects)
    ###################
    # POST-PROCESSING
    ###################
    seg = remove_small_objects(
        bw_masked, min_size=blob_min_area, connectivity=1, in_place=False
    )
