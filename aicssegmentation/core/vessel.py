...
def blobness3D(nd_array, scale_range=(1, 10), scale_step=2, tau=0.5, whiteonblack=True):
    ...
    for i, sigma in enumerate(sigmas):
        ...
        # print(eigenvalues[1])
        # print(eigenvalues[2])
        filtered_array[i] = compute_blobness3D(
            eigenvalues[0], eigenvalues[1], eigenvalues[2], tau=tau
        )
...
def plateness3D(
    ...
    for i, sigma in enumerate(sigmas):
        ...
        filtered_array[i] = compute_plateness3D(
            eigenvalues[0], eigenvalues[1], eigenvalues[2], pa, pb, pc, tau=tau
        )
...
def vesselness2D_range(
    nd_array, scale_range=(1, 10), scale_step=2, tau=0.5, whiteonblack=True
):
...
def compute_plateness3D(eigen1, eigen2, eigen3, pa, pb, pc, tau):
    ...
    response = (
        eigen2 - eigen3
    )  # -1*(eigen3 - eigen2) # assume eigen3<0, abs(eigen3)>abs(eigen2)
